...
class MfMaterial(Material):
    ...
    def write_serpent_matcard(self, **kwargs: 'name, color'):
        mcard = self.serpent_material_card().splitlines()
        color = 'rgb '+_colkwargs(kwargs.get('color', (0, 128, 128)))
        name = kwargs.get('name', self.name)
        density = '-'+str(self.rho)
        mcard[0] = ' '.join(['mat', name, density, color])
        return '\n'.join(mcard)
...
class test_monkeypatch(unittest.TestCase):
    ...
    def test_EtoE(self):
        ...
        mcew = E.write_serpent_matcard().splitlines()[1:]
        self.assertTrue(jse==mcew)
...
if __name__ is '__main__':
    unittest.main()
    EUROfer = EUROfer()
    a = EUROfer.write_serpent_matcard(name='test', color=np.random.rand(3)*255)
